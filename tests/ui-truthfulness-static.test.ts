...
describe('UI truthfulness static guards', () => {
  ...
  it('billing list action is labeled truthfully', () => {
    ...
    // KPI card shows "Open Cycles" with sub-label "OPEN + LOCKED"
    // The page uses openCycles count and labels it as "รอบบิลที่เปิด" (open cycles)
    expect(source).toMatch(/openCycles|Open Cycles|รอบบิลที่เปิด/);
    expect(source).toContain('OPEN + LOCKED');
