...
export function PaymentReviewPanel() {
  ...
  if (loading) {
    return (
      <div className="border rounded p-4">
        <div className="text-lg font-semibold mb-2">Payment Review Panel</div>
  ...
  if (transactions.length === 0) {
    return (
      <div className="border rounded p-4">
        <div className="text-lg font-semibold mb-2">Payment Review Panel</div>
        <p className="text-gray-500 text-center py-8">No transactions need review</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="border rounded p-4">
        <div className="text-lg font-semibold mb-4">Payment Review Panel</div>
          <div className="space-y-4">
              ...
              <div key={transaction.id} className="border rounded-lg p-4">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`px-2 py-1 rounded text-xs ${getConfidenceColor(transaction.confidenceScore)}`}>
                    {transaction.confidenceScore ? `${Math.round(transaction.confidenceScore * 100)}%` : 'Unknown'}
                  </span>
                </div>
                ...
                {transaction.invoice && (
                  <div className="bg-blue-50 p-3 rounded-md mb-3">
                    <div className="text-sm font-medium mb-1">Matched Invoice</div>
                    <div className="text-sm text-gray-700">
                      <div>Invoice: {transaction.invoice.invoiceNumber}</div>
                      <div>Room: {transaction.invoice.room.roomNumber}</div>
                      <div>Amount: {formatCurrency(transaction.invoice.total)}</div>
                      {transaction.invoice.room.roomTenants[0] && (
                        <div>Tenant: {transaction.invoice.room.roomTenants[0].tenant.firstName} {transaction.invoice.room.roomTenants[0].tenant.lastName}</div>
                      )}
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {processing === transaction.id ? 'Processing...' : 'Confirm Match'}
                  </button>
                  <button
          ...
          <div className="flex justify-between items-center mt-6">
            ...
            <span className="text-sm text-gray-600">
              Showing {offset + 1} - {Math.min(offset + limit, transactions.length + offset)} of {transactions.length}
            </span>
            <button
