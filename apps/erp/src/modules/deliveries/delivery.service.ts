...
export class DeliveryService {
  ...
  // Send a single document (from chat page) - creates ad-hoc order for tracking
  async sendSingleDocument(generatedDocumentId: string, actorId?: string | null) {
    ...
    if (!pdfFile) throw new Error('ไม่พบไฟล์ PDF ของเอกสารนี้');
    ...
    if (!lineUserId) throw new Error('ไม่พบ LINE ID ของผู้เช่าห้องนี้');

    // Create ad-hoc delivery order
    const order = await prisma.deliveryOrder.create({
      data: {
        channel: 'LINE',
