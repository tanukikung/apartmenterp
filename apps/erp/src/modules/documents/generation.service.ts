...
export class DocumentGenerationService {
  ...
  private async convertHtmlOutputs(
    ...
    if (canConvertWithOnlyOffice()) {
      pdfBuffer = await convertOnlyOfficeDocument({
        url: htmlFileUrl,
        fileType: 'html',
        outputType: 'pdf',
        title: `${title}.html`,
        key: createOnlyOfficeConversionKey(keyBase, 'pdf'),
      });

      docxBuffer = await convertOnlyOfficeDocument({
        url: htmlFileUrl,
        fileType: 'html',
        outputType: 'docx',
        title: `${title}.html`,
        key: createOnlyOfficeConversionKey(keyBase, 'docx'),
      }).catch(() => null);
    } else {
      pdfBuffer = Buffer.from(await generateDocumentPdf(title, html));
    }
