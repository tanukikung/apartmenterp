  ...
  try {
    ...
    for (const row of allRows) {
      try {
        ...
        // Update room status if rent = 0 (vacant)
        if (row.roomStatus === 'INACTIVE' && room.roomStatus !== 'INACTIVE') {
          await prisma.room.update({
            where: { roomNo: row.roomNo },
            data: { roomStatus: 'INACTIVE' },
          });
        } else if (row.roomStatus === 'ACTIVE' && room.roomStatus === 'INACTIVE') {
          // Reactivate room if it was marked inactive but now has rent
          await prisma.room.update({
            where: { roomNo: row.roomNo },
        ...
        errors++;
        console.error(`Error importing row for room ${row.roomNo}:`, err);
      }
    }
