...
import { generateRoomNumbers, RoomNumberFormat, type MixedSpecialFloor } from '@/app/admin/setup/lib/room-number-generator';
...
const mixedSpecialFloorSchema = z.object({
...
const setupCompleteSchema = z.object({
  ...
  rooms: z.object({
    format: z.enum(['SIMPLE', 'HOTEL', 'CUSTOM_PREFIX', 'MIXED']),
    floors: z.number().int().min(1).max(20),
    roomsPerFloor: z.number().int().min(1).max(50),
    defaultRentAmount: z.number().min(0),
    defaultRuleCode: z.string().default('STANDARD'),
    prefix: z.string().optional(),
    mixedSpecialFloor: mixedSpecialFloorSchema.optional(),
  }),
  billing: z.object({
...
export const POST = asyncHandler(async (req: NextRequest): Promise<NextResponse> => {
  ...
  if (body.rooms.prefix) {
    roomOptions.prefix = body.rooms.prefix;
  }
  if (body.rooms.mixedSpecialFloor) {
    roomOptions.mixedSpecialFloor = body.rooms.mixedSpecialFloor as MixedSpecialFloor;
  }

  const roomNumbers = generateRoomNumbers(
    body.rooms.format as RoomNumberFormat,
    body.rooms.floors,
    body.rooms.roomsPerFloor,
    roomOptions
  );

  // Create bank accounts (one per floor)
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // 4. Create rooms
    const roomsToCreate = roomNumbers.map((roomNo, index) => {
      let floorNo: number;

      if (body.rooms.format === 'MIXED' && body.rooms.mixedSpecialFloor) {
        // For MIXED format:
        // - Special floor rooms come first in the array
        // - Then normal floors (2,3,4..., skipping special floor)
        const specialFloorNo = body.rooms.mixedSpecialFloor.floorNo;
        const specialFloorRoomCount = body.rooms.mixedSpecialFloor.roomNumbers.length;
        const normalRoomsPerFloor = body.rooms.roomsPerFloor;

        if (index < specialFloorRoomCount) {
          // First batch = special floor
          floorNo = specialFloorNo;
        } else {
          // After special floor, remaining rooms go to floors 2,3,4...8
          const normalIndex = index - specialFloorRoomCount;
          const normalFloorIndex = Math.floor(normalIndex / normalRoomsPerFloor);
          floorNo = specialFloorNo + 1 + normalFloorIndex;
        }
      } else {
        // Standard calculation for SIMPLE, HOTEL, CUSTOM_PREFIX
        floorNo = Math.floor(index / body.rooms.roomsPerFloor) + 1;
      }

      const accountId = `ACC_F${floorNo}`;
      return {
        roomNo,
        floorNo,
        defaultAccountId: accountId,
        defaultRuleCode: body.rooms.defaultRuleCode,
        defaultRentAmount: body.rooms.defaultRentAmount,
        hasFurniture: false,
