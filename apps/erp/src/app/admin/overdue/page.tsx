...
type OverdueInvoice = {
  ...
  contract?: { tenant?: { firstName: string; lastName: string } | null } | null;
  tenantName?: string;
  roomNumber?: string;
};
...
function exportCsv(invoices: OverdueInvoice[]) {
  ...
  const rows = invoices.map((inv) => {
    ...
    return [roomNum(inv), tenantName(inv), inv.invoiceNumber ?? inv.id, new Date(inv.dueDate).toLocaleDateString('en-GB'), String(days), String(inv.totalAmount)];
  });
...
export default function AdminOverduePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setInvoices(raw);
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to load overdue invoices'); }
  ...
  const kpi = useMemo(() => {
    ...
    const avgDays = total > 0 ? Math.round(invoices.reduce((s, inv) => s + daysSince(inv.dueDate), 0) / total) : 0;
  ...
  async function sendAllReminders() {
    ...
    setWorking(null);
    setMessage(`Reminders sent: ${sent} succeeded${failed > 0 ? `, ${failed} failed` : ''}`);
  }
                  ...
                  filtered.map((inv) => {
                    ...
                    return (
                      <tr key={inv.id} className="border-b border-outline-variant/5 hover:bg-surface-container/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-on-surface-variant">{new Date(inv.dueDate).toLocaleDateString('en-GB')}</td>
                        <td className="px-4 py-3">
