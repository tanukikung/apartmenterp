    ...
    <main className="space-y-6">
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-primary-container to-primary px-6 py-5 shadow-lg">
        ...
        <div className="relative flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-base font-semibold text-on-primary">Batch Detail</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          ...
          <div className="font-semibold text-on-surface">Batch not found</div>
        </div>
      ) : (
        <>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5">
              <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-on-surface-variant">Filename</div>
              <div className="mt-2 text-sm font-medium text-on-surface">{batch.filename}</div>
            </div>
            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5">
              <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-on-surface-variant">Status</div>
              <div className="mt-2">
                <span className={statusBadge(batch.status)}>{batch.status}</span>
              </div>
            </div>
            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5">
              <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-on-surface-variant">Rows</div>
              <div className="mt-2 text-xl font-semibold text-on-surface">{totals.rowCount}</div>
            </div>
            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5">
              <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-on-surface-variant">Batch Total</div>
              <div className="mt-2 text-xl font-semibold text-on-surface">{money(totals.totalAmount)}</div>
            </div>
            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5">
              <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-on-surface-variant">Valid / Error</div>
          ...
          <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant px-5 py-4">
                <div className="text-sm font-semibold text-on-surface">Staged Rows</div>
                ...
                <table className="w-full text-sm text-left">
                  <thead>
                    <tr className="bg-surface-container">
                      ...
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">Sheet</th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">Total</th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">Check Notes</th>
                    </tr>
                  </thead>
              ...
              {batch.warningRows > 0 || batch.invalidRows > 0 ? (
                <section className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5">
                  ...
                  <p className="text-sm text-on-surface-variant">
                    This batch contains {batch.warningRows} warning row{batch.warningRows === 1 ? '' : 's'} and {batch.invalidRows} blocking error row{batch.invalidRows === 1 ? '' : 's'}. Correct the source file and re-upload.
                  </p>
                </section>
              ) : null}
            </div>
