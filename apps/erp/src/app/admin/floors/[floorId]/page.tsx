    ...
    async function load() {
      ...
      try {
        const [floorsRes, roomsRes] = await Promise.all([
          ...
          fetch(`/api/rooms?floorId=${floorId}&pageSize=100`, { cache: 'no-store' }).then((r) => r.json()),
        ]);
        ...
        {loading ? (
          ...
          filteredRooms.map((room) => (
            <Link
              ...
              className={`group flex flex-col rounded-2xl border p-3 shadow-sm transition-all hover:shadow-md ${STATUS_CARD_STYLE[room.status]}`}
            >
              {/* Room number */}
              <div className={`mb-1.5 text-lg font-bold leading-tight ${STATUS_NUMBER_COLOR[room.status]}`}>
                {room.roomNumber}
              </div>

              {/* Status badge */}
              <span
                className={`mb-2 inline-flex w-fit items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${STATUS_BADGE_STYLE[room.status]}`}
              >
