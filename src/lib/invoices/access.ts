...
export function verifySignedInvoiceAccess(input: {
  ...
  // Re-compute HMAC with the provided invoiceId and verify it matches the token.
  // Since the token is HMAC(secret, "invoiceId:action:expiresAt"), if an attacker
  // reuses a token for a different invoiceId, the recomputed HMAC will differ.
  const secret = resolveInvoiceAccessSecret()!;
  const payloadString = `${input.invoiceId}:${input.action}:${input.expiresAt}`;
