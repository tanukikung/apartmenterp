...
export const POST = asyncHandler(async (req: NextRequest, context?: Params): Promise<NextResponse> => {
  ...
  if (!room) {
    throw new BadRequestError(
      `Claimed room "${claimedRoomNo}" does not match any active room. Request a correction before approving.`
    );
  }
  ...
  if (room.tenants.length >= room.maxResidents) {
    throw new BadRequestError(
      `Room ${room.roomNo} is full (${room.maxResidents}/${room.maxResidents} occupants). Cannot approve additional registration.`
    );
  }
