...
export const POST = asyncHandler(async (
  ...
  const { allowed, remaining, resetAt } = await limiter.check(`template-version:${ip}`, ADMIN_MAX_ATTEMPTS, ADMIN_WINDOW_MS);
  if (!allowed) {
  ...
  const result = await service.createDraftVersionFromActive(params.id, session.sub);
