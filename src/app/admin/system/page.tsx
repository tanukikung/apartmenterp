    ...
    <main className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-primary-container to-primary px-6 py-5 shadow-lg">
        ...
        <div className="relative flex items-center justify-between">
          <div>
            <h1 className="text-base font-semibold text-on-primary">ระบบ</h1>
            <p className="text-xs text-on-primary/80 mt-0.5">ตรวจสอบสถานะแอปพลิเคชัน เวิร์คเกอร์ ความลึกคิว และความพร้อมการสำรองข้อมูล</p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5 hover:shadow-lg transition-shadow">
          <div className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">สถานะระบบ</div>
          <div className="mt-3">
          ...
          <div className="mt-3 text-sm text-on-surface-variant">สภาพแวดล้อม: {healthData?.services?.env || '-'} · DB: {healthData?.services?.database || '-'}</div>
        </div>
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5 hover:shadow-lg transition-shadow">
          <div className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">เวิร์คเกอร์</div>
          <div className="mt-3">
          ...
          <div className="mt-3 text-sm text-on-surface-variant">เช็คสถานะ: {deepData?.services?.worker?.lastHeartbeatMsAgo ?? '-'} มิลลิวินาทีที่แล้ว</div>
        </div>
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5 hover:shadow-lg transition-shadow">
          <div className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">คิวออกบอกซ์</div>
          <div className="mt-3 text-2xl font-extrabold text-primary">{metricsData?.outbox?.queueLength ?? 0}</div>
          <div className="mt-2 text-sm text-on-surface-variant">อีเวนต์ที่ล้มเหลว: {metricsData?.outbox?.failedCount ?? 0}</div>
        </div>
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 p-5 hover:shadow-lg transition-shadow">
          <div className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant">อัตราจับคู่การชำระ</div>
          <div className="mt-3 text-2xl font-extrabold text-primary">{Math.round((metricsData?.payments?.matchRate ?? 0) * 100)}%</div>
          <div className="mt-2 text-sm text-on-surface-variant">ตรวจสอบด้วยตนเอง: {metricsData?.payments?.manualReviewCount ?? 0}</div>
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-3">
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 overflow-hidden">
          <div className="px-5 py-4 border-b border-outline-variant/10">
            <div className="text-sm font-semibold text-primary">สถานะแอปพลิเคชัน</div>
          </div>
        ...
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 overflow-hidden">
          <div className="px-5 py-4 border-b border-outline-variant/10">
            <div className="text-sm font-semibold text-primary">เวิร์คเกอร์และคิว</div>
          </div>
          <div className="space-y-2 p-4 text-sm text-on-surface">
        ...
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 overflow-hidden">
          <div className="px-5 py-4 border-b border-outline-variant/10">
            <div className="text-sm font-semibold text-primary">สถานะการสำรองข้อมูล</div>
          </div>
          <div className="space-y-2 p-4 text-sm text-on-surface">
            ...
            <div>ไดเรกทอรีสำรอง: <span className="font-mono text-xs text-on-surface-variant">{backupData?.dir || '-'}</span></div>
