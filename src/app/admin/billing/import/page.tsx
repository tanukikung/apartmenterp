    ...
    <main className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-xl bg-[hsl(var(--color-surface))] backdrop-blur border border-[hsl(var(--color-border))] px-6 py-5 shadow-[var(--glow-primary)]">
        ...
        <div className="relative flex items-center justify-between">
          <div>
            <h1 className="text-base font-semibold text-[hsl(var(--color-text))]">นำเข้าการเรียกเก็บ</h1>
          ...
          <div className="flex items-center gap-3">
            <a href="/billing_template.xlsx" download="billing_template.xlsx" className="inline-flex items-center gap-2 rounded-xl border border border-[hsl(var(--color-border))]/80 bg-[hsl(var(--color-surface))]/20 backdrop-blur px-4 py-2 text-sm font-medium text-[hsl(var(--color-text))]/80 shadow-sm transition-all hover:bg-white/[0.1] hover:border-white/20 active:scale-[0.98]">
              <FileSpreadsheet className="h-4 w-4" />
              ดาวน์โหลด Template
            </a>
            <Link href="/admin/billing/batches" className="inline-flex items-center gap-2 rounded-xl border border border-[hsl(var(--color-border))]/80 bg-[hsl(var(--color-surface))]/20 backdrop-blur px-4 py-2 text-sm font-medium text-[hsl(var(--color-text))]/80 shadow-sm transition-all hover:bg-white/[0.1] hover:border-white/20 active:scale-[0.98]">
              ดูแบทช์
            </Link>
          </div>
        </div>
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all active:scale-[0.98] ${
                importMode === 'template'
                  ...
                  : 'bg-[hsl(var(--color-surface))] text-[hsl(var(--color-text))]/50 hover:bg-white/[0.08] hover:text-[hsl(var(--color-text))]/80'
              }`}
            >
          ...
          <div className="px-5 py-4 border-b border-[hsl(var(--color-border))] bg-amber-500/5">
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2">
                <label className="text-sm font-medium text-[hsl(var(--color-text))]/70">เดือน:</label>
                <select
                  value={selectedMonth}
              ...
              <div className="flex items-center gap-2">
                <label className="text-sm font-medium text-[hsl(var(--color-text))]/70">ปี:</label>
                <select
                  value={selectedYear}
              ...
              <div className="text-sm text-[hsl(var(--color-text))]/40">
                ดาวน์โหลด template:{' '}
      ...
      {/* Upload Section */}
      <section className="bg-[hsl(var(--color-surface))] backdrop-blur border border-[hsl(var(--color-border))] rounded-2xl">
        ...
        <div className="grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div
            ...
            className={`flex min-h-[220px] cursor-pointer flex-col items-center justify-center rounded-[2rem] border-2 border-dashed px-6 py-10 text-center transition-all active:scale-[0.98] ${
              file
                ...
                : 'border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))]/20 hover:border-[hsl(var(--primary))]/30 hover:bg-[hsl(var(--primary))]/5'
            }`}
          >
            <input
              ref={fileRef}
            ...
            />
            <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-[1.5rem] bg-white/5 border border border-[hsl(var(--color-border))]/80 shadow-sm">
              {file ? (
                <CheckCircle2 className="h-8 w-8 text-emerald-400" />
              ) : (
                <UploadCloud className="h-8 w-8 text-[hsl(var(--color-text))]/30" />
              )}
            </div>
            <div className="text-lg font-semibold text-[hsl(var(--color-text))]">
              {file ? file.name : 'Drop or choose the Excel file'}
            </div>
            <p className="mt-2 max-w-md text-sm text-[hsl(var(--color-text))]/40">
              {importMode === 'template' ? (
                <>
          ...
          <div className="space-y-4 rounded-[2rem] border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))]/20 backdrop-blur p-5">
            <div>
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-[hsl(var(--color-text))]/40">
                Workflow
              </div>
              <div className="mt-3 space-y-3 text-sm text-[hsl(var(--color-text))]/70">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-4 py-2 text-sm font-semibold text-[hsl(var(--color-text))] shadow-glow-primary transition-all hover:bg-[hsl(var(--color-primary-dark))] hover:shadow-glow-primary-hover active:scale-[0.98] flex flex-1 items-center justify-center gap-2 disabled:opacity-40"
              >
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileSpreadsheet className="h-4 w-4" />}
                {loading ? 'Previewing...' : 'Preview Batch'}
              </button>
              <button type="button" onClick={resetAll} className="inline-flex items-center gap-2 rounded-xl border border border-[hsl(var(--color-border))]/80 bg-[hsl(var(--color-surface))] backdrop-blur px-4 py-2 text-sm font-medium text-[hsl(var(--color-text))]/70 transition-all hover:bg-white/[0.08] hover:border-white/20 active:scale-[0.98]">
                Reset
              </button>
            </div>
          </div>
        ...
        <section className="space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            <div className="bg-[hsl(var(--color-surface))] backdrop-blur border border-[hsl(var(--color-border))] rounded-xl p-5">
              <div className="text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">รหัสแบทช์</div>
              <div className="mt-2 font-mono text-xs text-[hsl(var(--primary))]">{preview.batch.id}</div>
            </div>
            <div className="bg-[hsl(var(--color-surface))] backdrop-blur border border-[hsl(var(--color-border))] rounded-xl p-5">
              <div className="text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">รอบการเรียกเก็บ</div>
              <div className="mt-2 text-sm font-semibold text-[hsl(var(--color-text))]/80">{preview.batch.billingCycleId.slice(0, 8)}…</div>
            </div>
            <div className="bg-[hsl(var(--color-surface))] backdrop-blur border border-[hsl(var(--color-border))] rounded-xl p-5">
              <div className="text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">ห้อง</div>
              <div className="mt-2 text-sm font-semibold text-[hsl(var(--color-text))]">{totals.rooms}</div>
            </div>
            <div className="bg-[hsl(var(--color-surface))] backdrop-blur border border-[hsl(var(--color-border))] rounded-xl p-5">
              <div className="text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">ถูกต้อง / ข้อผิดพลาด</div>
              <div className="mt-2 text-sm font-semibold text-[hsl(var(--color-text))]">
                {preview.batch.validRows} / {preview.batch.invalidRows}
              </div>
            </div>
            <div className="bg-[hsl(var(--color-surface))] backdrop-blur border border-[hsl(var(--color-border))] rounded-xl p-5">
              <div className="text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">ยอดรวมแบทช์</div>
              <div className="mt-2 text-sm font-semibold text-[hsl(var(--color-text))]">{money(totals.totalAmount)}</div>
            </div>
          </div>

          {/* Warnings */}
          {preview.warnings.length > 0 ? (
            <section className="bg-[hsl(var(--color-surface))] backdrop-blur border border-amber-500/20 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-[hsl(var(--color-border))]">
              ...
              <div className="overflow-auto">
                <table className="w-full text-sm text-left">
                  <thead>
                    <tr className="bg-[hsl(var(--color-surface))]/20">
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">ห้อง</th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">ประเภท</th>
                      <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">ปัญหา</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/[0.05]">
                    {preview.warnings.map((warning) => (
                      <tr key={`${warning.roomNumber}-${warning.year}-${warning.month}`} className="hover:bg-[hsl(var(--color-surface))] transition-colors">
                        <td className="font-semibold text-[hsl(var(--color-text))]/90">{warning.roomNumber}</td>
                        <td>
          ...
          <section className="bg-[hsl(var(--color-surface))] backdrop-blur border border-[hsl(var(--color-border))] rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-[hsl(var(--color-border))]">
              ...
              <div className="flex items-center gap-2 mt-2">
                <Link href={`/admin/billing/batches/${preview.batch.id}`} className="inline-flex items-center gap-2 rounded-lg border border border-[hsl(var(--color-border))]/80 bg-[hsl(var(--color-surface))] backdrop-blur px-4 py-2 text-sm font-medium text-[hsl(var(--color-text))]/70 transition-all hover:bg-white/[0.08] hover:border-white/20 active:scale-[0.98]">
                  เปิดรายละเอียดแบทช์
                </Link>
              </div>
            </div>
              ...
              <table className="w-full text-sm text-left">
                <thead>
                  <tr className="bg-[hsl(var(--color-surface))]/20">
                    ...
                    <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">รวม</th>
                    <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[hsl(var(--color-text))]/40">ตรวจสอบ</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.05]">
                  {preview.preview.map((group) => {
                    ...
                    return (
                      <tr key={`${group.roomNumber}-${group.year}-${group.month}`} className="hover:bg-[hsl(var(--color-surface))] transition-colors">
                        <td className="font-semibold text-[hsl(var(--color-text))]/90">{group.roomNumber}</td>
                        <td className="text-[hsl(var(--color-text))]/50">
                          {group.month}/{group.year}
                        </td>
                        <td className="text-[hsl(var(--color-text))]/50">{group.count}</td>
                        <td className="text-[hsl(var(--color-text))]/90 font-semibold">{money(group.total)}</td>
                        <td>
                          {groupWarnings ? (
          ...
          {/* Commit section */}
          <section className="bg-[hsl(var(--color-surface))] backdrop-blur border border-[hsl(var(--color-border))] rounded-xl">
            <div className="flex flex-wrap items-center justify-between gap-3 p-5">
              <div>
                <div className="text-base font-semibold text-[hsl(var(--color-text))]">2. Commit Staged Batch</div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  onClick={() => void handlePreview()}
                  disabled={loading}
                  className="inline-flex items-center gap-2 rounded-xl border border border-[hsl(var(--color-border))]/80 bg-[hsl(var(--color-surface))] backdrop-blur px-4 py-2 text-sm font-medium text-[hsl(var(--color-text))]/70 transition-all hover:bg-white/[0.08] hover:border-white/20 active:scale-[0.98]"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-4 py-2 text-sm font-semibold text-[hsl(var(--color-text))] shadow-glow-primary transition-all hover:bg-[hsl(var(--color-primary-dark))] hover:shadow-glow-primary-hover active:scale-[0.98] disabled:opacity-40"
                >
      ...
      {result ? (
        <section className="bg-[hsl(var(--color-surface))] backdrop-blur border border-emerald-500/20 rounded-xl">
          <div className="flex flex-col gap-5 p-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-4">
              <div className="flex h-14 w-14 items-center justify-center rounded-[1.5rem] bg-emerald-500/20 border border-emerald-500/30 shadow-sm">
                <CheckCircle2 className="h-7 w-7 text-emerald-400" />
              </div>
              <div>
                <h2 className="text-xl font-semibold text-emerald-400">นำเข้าเสร็จสมบูรณ์</h2>
                <p className="mt-1 text-sm text-[hsl(var(--color-text))]/50">
                  สร้าง {result.totalImported} บันทึกการเรียกเก็บจากแบทช์ {result.batchId}
                </p>
              </div>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link href={`/admin/billing/${result.cycleId}`} className="inline-flex items-center gap-2 rounded-xl border border border-[hsl(var(--color-border))]/80 bg-[hsl(var(--color-surface))] backdrop-blur px-4 py-2 text-sm font-medium text-[hsl(var(--color-text))]/70 transition-all hover:bg-white/[0.08] hover:border-white/20 active:scale-[0.98]">
                Open Billing Cycle
              </Link>
              <Link href={`/admin/billing/batches/${result.batchId}`} className="inline-flex items-center gap-2 rounded-xl border border border-[hsl(var(--color-border))]/80 bg-[hsl(var(--color-surface))] backdrop-blur px-4 py-2 text-sm font-medium text-[hsl(var(--color-text))]/70 transition-all hover:bg-white/[0.08] hover:border-white/20 active:scale-[0.98]">
                Open Batch Detail
              </Link>
            </div>
          </div>
      ...
      {confirmDialogOpen && preview ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          {/* Dialog */}
          <div className="relative z-10 w-full max-w-lg rounded-2xl border border-white/[0.10] bg-[hsl(var(--color-surface))]/95 backdrop-blur p-6 shadow-2xl">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-[hsl(var(--color-text))]">ยืนยันการนำเข้า?</h3>
                <p className="mt-1 text-sm text-[hsl(var(--color-text))]/50">
              ...
              <button
                onClick={() => setConfirmDialogOpen(false)}
                className="text-[hsl(var(--color-text))]/40 hover:text-[hsl(var(--color-text))]/70 transition-colors"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-3">
              <div className="rounded-lg border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))]/20 p-3 text-center">
                <div className="text-xs text-[hsl(var(--color-text))]/40">ห้อง</div>
                <div className="mt-1 text-lg font-semibold text-[hsl(var(--color-text))]">{totals.rooms}</div>
              </div>
              <div className="rounded-lg border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))]/20 p-3 text-center">
                <div className="text-xs text-[hsl(var(--color-text))]/40">ยอดรวม</div>
                <div className="mt-1 text-lg font-semibold text-[hsl(var(--color-text))]">{money(totals.totalAmount)}</div>
              </div>
              <div className="rounded-lg border border-[hsl(var(--color-border))] bg-[hsl(var(--color-surface))]/20 p-3 text-center">
                <div className="text-xs text-[hsl(var(--color-text))]/40">คำเตือน</div>
